...
public class STokenAdminSetCommand extends BaseCommand {

    private SToken sToken;
    YamlConfiguration cfg;

    public STokenAdminSetCommand(SToken sToken) {
    ...
    public void executeBoth(CommandSender sender, String[] args) {
        ...
        if(args.length == 2) {
            ...
            //- Adding
            sToken.getTEconomy().setBalance(target, amount);

            //- Messaging
            sender.sendMessage(TUtils.c(cfg.getString("messages.set-confirm-sender")
                    .replace("%token_amount%", TUtils.formatNumber(amount))
                    .replace("%player%", target.getName())
            ));

            if(target.isOnline()) {
                target.getPlayer().sendMessage(TUtils.c(cfg.getString("messages.set-confirm-receiver")
                        .replace("%token_amount%", TUtils.formatNumber(amount))
                ));
            }
